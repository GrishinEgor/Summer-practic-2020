...
class Monitoring():
    ...
    def __init__(self, f_print):
        ...
        try:
            ...
            print('Неожиданная ошибка при подготовке и проверке путей: {0}'.format(err), file=self.f_print)
        ...
        for i in range(self.num_parts):
            if arr_active_part[i]:
                ...
                try:
                    ...
                    time_end = time.time()
                    if time_end - time_start < 1:
                        print('\nВыполнение пункта {} заняло: {:.3f} сек.'.format(self.name_parts[i], time_end - time_start), file=self.f_print)
                    else:
                        print('\nВыполнение пункта {} заняло: {:.3f} мин.'.format(self.name_parts[i], (time_end - time_start)/60), file=self.f_print)
                    print('----------------------------------------------------', file=self.f_print)
    ...
    def part1(self):
        print("Выполняется создание и занесение выборки", file=self.f_print)
        # raise Exception("Какая-то ошибка")
        # time.sleep(2)
